...
public class IGetCurrentNewsImpl implements IGetCurrentNews {
	...
	public photoModel GetCurrentNewsPhoto(String newsID) throws SQLException {
		...
		if(news!=null)
		{
			...
			ResultSet rs = this.imysql.selectSQL("select * from photos where name='"+news.getPhotoID()+"';");
			
			if(rs.next())
			{
				photo.setPhotoName(rs.getString("name"));
				photo.setPhotoAbstract(rs.getString("abstract"));
